...
const Ventas = () => {
  ...
  const elementosPorPagina = 3; // Number of sales per page
  ...
  const eliminarVenta = async () => {
    ...
    try {
      ...
      setErrorCarga(null);
      setPaginaActual(1); // Reset to first page after deletion
    } catch (error) {
      setErrorCarga(error.message);
    }
  };
  ...
  const agregarVenta = async () => {
    ...
    try {
      ...
      setErrorCarga(null);
      setPaginaActual(1); // Reset to first page after adding a sale
    } catch (error) {
      setErrorCarga(error.message);
    }
  };
  ...
  const actualizarVenta = async (ventaActualizada, detalles) => {
    ...
    try {
      ...
      setDetallesEditados([]);
      setErrorCarga(null);
      setPaginaActual(1); // Reset to first page after updating a sale
    } catch (error) {
      setErrorCarga(error.message);
    }
  };
  ...
  const ventasPaginadas = listaVentas.slice(
